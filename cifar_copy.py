...
def graph_vgg(pr_target):    
    ...
    for name, module in origin_model.named_modules():
        if isinstance(module, nn.Conv2d):
            ...
            #conv_weight = torch.div(conv_params[index],math.pow(flops_cfg['vgg_cifar'][index],flops_lambda['vgg_cifar']))
            weights.append(conv_weight.view(-1))
            # print(module.weight.data.median())
            index += 1
    # with open('/home/xuchi/projects/swa_gaussian/vgg_conv_params.pkl', 'rb') as f:
    #     conv_params = pickle.load(f)
    # weights = []
    ...
    for name, module in origin_model.named_modules():
        ...
        if isinstance(module, nn.Conv2d):
            ##get_prior
            conv_weight = module.weight.data
            weight_vec = conv_weight.view(conv_weight.size()[0],-1).cpu().numpy()
            similar_matrix = distance.cdist(weight_vec, weight_vec, 'euclidean') ##can use softmax here
